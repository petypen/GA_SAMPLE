...
// Функция - Подготовка данных для GA о переходе на форму
//
// Параметры:
...
//
&НаКлиенте
Функция GA_Форма(сОбработкаНаименование, сФормаНаименование, мДанные = Неопределено) Экспорт
	Перем стДанные;
...
Процедура ОбязательныеПараметры_GA(стДанные)
	...
	стДанные.Вставить("an",  НаименованиеКонфигурации());
	стДанные.Вставить("av",  ВерсияКонфигурации());
	стДанные.Вставить("cd1", ВерсияПлатформы());
	стДанные.Вставить("cd2", ВерсияОС());
	стДанные.Вставить("cd3", Обработка_Имя());
	стДанные.Вставить("cd4", ВерсияОбработки());
